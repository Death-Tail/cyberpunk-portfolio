...
interface TrackerItem {
  id: string
  section: "bought" | "wish"
  name: string; cat: string; price: number; note: string
  date?: string        // bought only
  priority?: "high" | "medium" | "low"  // wish only
  status?: "next" | "hold" | "tbd"      // wish only
  sort_order: number
}
...
function fmt(n: number) { return n > 0 ? Number(n).toLocaleString("en-IQ") + " IQD" : null }
...
async function apiFetch(method: string, body?: object, query?: string) {
  ...
  const endpoint = `/api/admin/tracker${query ?? ""}`
...
export default function TrackerPage() {
  ...
  const [editBI, setEditBI] = useState<TrackerItem | null>(null)
  const [bf, setBf] = useState({ name: "", cat: "Keyboard", price: "", note: "", date: today() })

  /* Wish form */
  const [showWF, setShowWF] = useState(false)
  const [editWI, setEditWI] = useState<TrackerItem | null>(null)
  const [wf, setWf] = useState<{ name: string; cat: string; price: string; note: string; priority: "high"|"medium"|"low"; status: "next"|"hold"|"tbd" }>({
    name: "", cat: "Case", price: "0", note: "", priority: "medium", status: "tbd",
  })
  ...
  /* ── Derived ── */
  const bought = items.filter(i => i.section === "bought")
  const wish = items.filter(i => i.section === "wish")
  const spent = bought.reduce((s, x) => s + (x.price || 0), 0)
  const planned = wish.filter(x => x.price > 0).reduce((s, x) => s + x.price, 0)
  ...
  function openEditBought(item: TrackerItem) {
    setEditBI(item)
    setBf({ name: item.name, cat: item.cat, price: String(item.price || ""), note: item.note, date: item.date ?? today() })
    setShowBF(true)
  }
  async function saveBought() {
    if (!bf.name.trim()) return
    const payload = { section: "bought", name: bf.name.trim(), cat: bf.cat, price: parseFloat(bf.price) || 0, note: bf.note.trim(), date: bf.date, sort_order: editBI?.sort_order ?? bought.length }
    try {
      if (editBI) await apiFetch("PUT", { id: editBI.id, ...payload })
      else await apiFetch("POST", payload)
      await refresh()
    } catch { /* empty */ }
    setShowBF(false); setEditBI(null)
  }
  async function deleteBought(item: TrackerItem) {
    if (!confirm(`Delete "${item.name}"?`)) return
    await apiFetch("DELETE", undefined, `?id=${item.id}`)
    await refresh()
  }
  async function moveBoughtToWish(item: TrackerItem) {
    await apiFetch("PUT", { id: item.id, section: "wish", priority: "medium", status: "tbd", date: null, sort_order: wish.length })
    await refresh()
  }
  ...
  function openEditWish(item: TrackerItem) {
    setEditWI(item)
    setWf({ name: item.name, cat: item.cat, price: String(item.price || 0), note: item.note, priority: item.priority ?? "medium", status: item.status ?? "tbd" })
    setShowWF(true)
  }
  async function saveWish() {
    if (!wf.name.trim()) return
    const payload = { section: "wish", name: wf.name.trim(), cat: wf.cat, price: parseFloat(wf.price) || 0, note: wf.note.trim(), priority: wf.priority, status: wf.status, date: null, sort_order: editWI?.sort_order ?? wish.length }
    try {
      if (editWI) await apiFetch("PUT", { id: editWI.id, ...payload })
      else await apiFetch("POST", payload)
      await refresh()
    } catch { /* empty */ }
    setShowWF(false); setEditWI(null)
  }
  async function deleteWish(item: TrackerItem) {
    if (!confirm(`Delete "${item.name}"?`)) return
    await apiFetch("DELETE", undefined, `?id=${item.id}`)
    await refresh()
  }
  async function wishToBought(item: TrackerItem) {
    await apiFetch("PUT", { id: item.id, section: "bought", date: today(), priority: null, status: null, sort_order: bought.length })
    await refresh()
  }
    ...
    return (
      <main className="fixed inset-0 bg-ink text-bone grid place-items-center grain">
        <span className="font-mono-tight text-[10px] uppercase tracking-[0.3em] text-bone-mute animate-pulse">
          {!authed ? "Checking session…" : "Loading tracker…"}
        </span>
      </main>
    )
  }
  ...
  return (
    <main className="min-h-screen bg-ink text-bone grain">
      ...
      <div className="relative w-full h-65 overflow-hidden">
        ...
        <div className="relative h-full flex flex-col justify-end px-8 lg:px-12 pb-8">
          ...
          <p className="font-mono-tight text-[10px] uppercase tracking-[0.3em] text-bone-mute mt-2">
            {itemCount} item{itemCount !== 1 ? "s" : ""} tracked · synced to cloud
          </p>
        </div>
      </div>
      ...
      <div className="max-w-215 mx-auto px-6 lg:px-8 pb-24">
        ...
        {/* ══ ACQUIRED ══ */}
        <SectionHead title="Acquired" count={bought.length} />
        <table className="w-full border-collapse mb-2">
          ...
          <tbody>
            {bought.length === 0 ? (
              ...
              <tr key={item.id} className="group transition-colors hover:bg-ember-glow">
                ...
                <td className="py-4 px-4 border-b border-line text-right font-mono-tight text-sm font-bold text-ember">
                  {fmt(item.price) || <span className="text-bone-mute">—</span>}
                </td>
                <td className="py-4 px-4 border-b border-line text-right whitespace-nowrap">
                  <IBtn icon={<Pencil size={13} />} onClick={() => openEditBought(item)} />
                  <IBtn icon={<ArrowUp size={13} />} onClick={() => moveBoughtToWish(item)} />
                  <IBtn icon={<Trash2 size={13} />} onClick={() => deleteBought(item)} danger />
                </td>
              </tr>
            ))}
          </tbody>
        ...
        {showBF && (
          <div className="bg-ink-2 border border-line-strong border-t-2 border-t-ember-soft p-5 mb-3">
            <div className="eyebrow text-ember! mb-4">— {editBI ? "edit" : "add acquired"} item</div>
            <div className="grid grid-cols-[2fr_1fr_1fr] gap-3 mb-3">
              ...
              <Field label="Price paid (IQD)" value={bf.price} onChange={v => setBf({ ...bf, price: v })} type="number" />
            </div>
            <div className="grid grid-cols-[2fr_1fr] gap-3 mb-4">
              <Field label="Note" value={bf.note} onChange={v => setBf({ ...bf, note: v })} placeholder="e.g. includes delivery…" />
        ...
        {/* ══ PLANNED ══ */}
        <SectionHead title="Planned" count={wish.length} />
        <table className="w-full border-collapse mb-2">
          <thead><tr>
          ...
          <tbody>
            {wish.length === 0 ? (
              ...
              <tr key={item.id} className={`group transition-colors hover:bg-ember-glow ${item.status === "hold" ? "opacity-60" : ""}`}>
                <td className="py-4 px-4 border-b border-line">
                ...
                <td className="py-4 px-4 border-b border-line text-right whitespace-nowrap">
                  <IBtn icon={<Pencil size={13} />} onClick={() => openEditWish(item)} />
                  <IBtn icon={<Check size={13} />} onClick={() => wishToBought(item)} />
                  <IBtn icon={<Trash2 size={13} />} onClick={() => deleteWish(item)} danger />
                </td>
              </tr>
            ))}
          </tbody>
        ...
        {showWF && (
          <div className="bg-ink-2 border border-line-strong border-t-2 border-t-ember-soft p-5 mb-3">
            <div className="eyebrow text-ember! mb-4">— {editWI ? "edit" : "add planned"} item</div>
            <div className="grid grid-cols-[2fr_1fr_1fr] gap-3 mb-3">
              ...
              <Field label="Est. price (0 = TBD)" value={wf.price} onChange={v => setWf({ ...wf, price: v })} type="number" />
            </div>
            <div className="grid grid-cols-[2fr_1fr] gap-3 mb-4">
        ...
        <footer className="border-t border-line-strong pt-6 flex items-center justify-between">
          <span className="font-mono-tight text-[9px] tracking-widest text-bone-mute">Synced to Supabase · private · admin only</span>
          <span className="font-mono-tight text-[9px] tracking-widest text-bone-mute">works on any device</span>
        </footer>
      </div>
...
function Th({ children, right }: { children: React.ReactNode; right?: boolean }) {
  return <th className={`font-mono-tight text-[9px] tracking-[0.16em] uppercase text-bone-mute font-normal pb-2.5 px-4 border-b border-line-strong ${right ? "text-right" : "text-left"}`}>{children}</th>
}
function CatBadge({ cat }: { cat: string }) {
  return <span className="font-mono-tight text-[8px] tracking-widest uppercase text-bone-dim border border-line-strong px-2 py-1 bg-ink-3">{cat}</span>
}
...
function IBtn({ icon, onClick, danger }: { icon: React.ReactNode; onClick: () => void; danger?: boolean }) {
  return <button onClick={onClick} className={`p-1.5 text-bone-mute transition-colors ${danger ? "hover:text-ember" : "hover:text-bone"}`}>{icon}</button>
}
...
function Btn({ children, primary, onClick }: { children: React.ReactNode; primary?: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`flex items-center gap-2 font-mono-tight text-[9px] tracking-[0.16em] uppercase px-5 py-2.5 border transition-all ${primary ? "bg-ember-soft border-ember-soft text-bone hover:bg-ember hover:border-ember" : "bg-transparent border-line-strong text-bone-mute hover:border-bone-dim hover:text-bone"}`}>
      {children}
    </button>
  )
}
