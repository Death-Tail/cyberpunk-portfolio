...
import logoImg from '@/public/LogoP.webp'
...
export function Taskbar({ windows, onOpenWindow, onFocusWindow, onMinimizeWindow }: TaskbarProps) {
  ...
  return (
    <>
      {/* Start Menu */}
      {startMenuOpen && (
        <div className="fixed bottom-12 left-0 flex flex-row items-stretch bg-slate-950/95 backdrop-blur-xl z-50 border border-teal-900/50 shadow-[0_0_40px_rgba(0,0,0,0.6)] rounded-tr-lg overflow-hidden animate-in slide-in-from-bottom-2 duration-200">
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
              {applications.map((app) => (
                <button
                  ...
                  className="w-full flex items-center p-2 hover:bg-white/5 transition-colors rounded group"
                >
                  <div className="mr-3 flex items-center justify-center w-8 h-8 bg-slate-800/50 rounded p-1 border border-white/5 group-hover:border-teal-500/30 transition-colors">
                  ...
                  <span className="text-slate-200 text-sm font-medium group-hover:text-teal-400 transition-colors">{app.name}</span>
                </button>
              ))}
            </div>

            <div className="p-2 border-t border-white/5 bg-black/20">
              <button className="w-full flex items-center p-2 hover:bg-red-500/10 hover:text-red-400 text-slate-400 transition-colors rounded mb-1">
          ...
          <div className="w-80 bg-black/20 p-4 flex flex-col">
                ...
                {contactLinks.map((item) => (
                    <a
                        ...
                        className={`
                            ...
                            bg-slate-800/40 border border-white/5 overflow-hidden
                            transition-all duration-300 group
                            hover:bg-slate-800/80 hover:border-teal-500/40 hover:shadow-[0_0_15px_rgba(20,184,166,0.1)]
                            ${item.preferred ? 'ring-1 ring-teal-500/30 bg-teal-900/10' : ''}
                        `}
                    >
                        {/* Preferred Badge */}
                        {item.preferred && (
                           <div className="absolute top-0 right-0 bg-teal-600 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-bl shadow-sm z-10">
                            PREFERED
                           </div>
                        )}

                        <div className="flex items-start justify-between z-10">
                            <div className="w-8 h-8 relative rounded-md overflow-hidden bg-black/20 p-1 border border-white/10 group-hover:border-teal-500/50 transition-colors">
                                <Image
                                  src={item.iconPath}
                                  alt={item.platform}
                                  width={32}
                            ...
                            <ExternalLink className="w-3 h-3 text-slate-600 opacity-0 group-hover:opacity-100 group-hover:text-teal-400 transition-all -translate-y-2 group-hover:translate-y-0" />
                        </div>
                        ...
                        <div className="z-10 relative">
                            ...
                            <div className="absolute inset-0 pt-1 opacity-0 translate-y-2 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-[-0.4rem]">
                                <p className="text-[10px] leading-tight text-teal-300 font-medium">
                                  {item.description}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="absolute inset-0 bg-linear-to-br from-transparent via-transparent to-teal-500/5 opacity-0 group-hover:opacity-100 transition-opacity" />
                    </a>
                ))}
            </div>
            ...
            <div className="mt-auto pt-4 border-t border-white/5 flex justify-between items-center">
                <div className="text-[10px] text-slate-500">
                    <span className="block">Logged in as Admin</span>
                </div>
                <Settings className="w-4 h-4 text-slate-600 hover:text-teal-400 cursor-pointer transition-colors" />
            </div>
          </div>
